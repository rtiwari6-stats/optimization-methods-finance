...
#workingFolder <- "C:/Users/arize/Documents/CFRM/CFRM507/2022/Project 1/SolveHere"
...
uniqueMarkets <- data.frame(Market = uniqueMarkets) %>% mutate(MarketIndex = row_number())

uniqueCountries <- unique(benchmark_df$Country)
uniqueCountries <- data.frame(Country = uniqueCountries) %>% mutate(CountryIndex = row_number())

uniqueCapTiers <- unique(benchmark_df$Cap.Tier)
uniqueCapTiers <- data.frame(Cap.Tier = uniqueCapTiers) %>% mutate(CapTierIndex = row_number())

uniqueSectors <- unique(benchmark_df$Sector)
uniqueSectors <- data.frame(Sector = uniqueSectors) %>% mutate(SectorIndex = row_number())
...
cbind(NonZeroHoldingIndexes,X_solution[NonZeroHoldingIndexes])
...
# cap tier weights
cbind(0.98 * CapTierWeight * W_solution, t(CapTier) %*% X_solution, 1.02 * CapTierWeight * W_solution)
